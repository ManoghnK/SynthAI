...
* **Specialized Quiz Model:** A dedicated model is initialized for quiz generation. The system attempts to load models in order of preference:
  ...
  4. `TheBloke/Llama-2-7B-Chat-GPTQ` (quantized version)
...
1. Installing `pyngrok` or `localtunnel`.

2. Authenticating with an `ngrok` authtoken.

3. Running the Streamlit app in the background.

4. Exposing the Streamlit app via a public URL.
